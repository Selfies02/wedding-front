...
const Videos: React.FC = () => {
  ...
  useEffect(() => {
    const fetchVideos = async () => {
      try {
        setLoading(true);
        const response = await fetch('http://3.21.100.147/api/upload-files/videos');
        ...
        if (data && data.videos && Array.isArray(data.videos)) {
          const videosList = data.videos.map((fileName: string, index: number) => {
            ...
            return {
              id: index + 1,
              url: `http://3.21.100.147/uploads/videos/${fileName}`,
              title,
              author,
            };
          });
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    try {
      ...
      xhr.open('POST', 'http://3.21.100.147/api/upload-files/files', true);
      xhr.send(formData);
    ...
    confirmModal.warning(
      ...
      async () => {
        try {
          ...
          const response = await fetch('http://3.21.100.147/api/upload-files/remove/video', {
            method: 'POST',
