...
const Gallery: React.FC = () => {
  ...
  useEffect(() => {
    const fetchPhotos = async () => {
      try {
        setLoading(true);
        const response = await fetch('http://localhost:3000/api/upload-files/photos');
        ...
        if (data && data.photos && Array.isArray(data.photos)) {
          const photosList = data.photos.map((fileName: string, index: number) => {
            ...
            return {
              id: index + 1,
              url: `http://localhost:3000/uploads/photos/${fileName}`,
              title,
              author,
            };
          });
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    try {
      ...
      xhr.open('POST', 'http://localhost:3000/api/upload-files/files', true);
      xhr.send(formData);
    ...
    confirmModal.warning(
      ...
      async () => {
        try {
          ...
          const response = await fetch('http://localhost:3000/api/upload-files/remove/photo', {
            method: 'POST',
